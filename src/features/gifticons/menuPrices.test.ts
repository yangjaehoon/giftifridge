...
describe('lookupEstimatedPrice', () => {
  ...
  it('is brand-specific: the same drink name resolves to that brand price', () => {
    ...
    expect(lookupEstimatedPrice('메가MGC커피', '(ICE)아메리카노')?.price).toBe(1500);
  });

  it('prefers the most specific keyword when several match', () => {
    // "아메리카노" and "아메리카노grande" both match; the longer one wins.
    expect(lookupEstimatedPrice('스타벅스', '카페 아메리카노 GRANDE')?.price).toBe(5000);
  });

  it('matches chicken by brand + signature menu', () => {
    expect(lookupEstimatedPrice('BHC', '뿌링클+콜라1.25L')?.price).toBe(20000);
    expect(lookupEstimatedPrice('교촌치킨', '허니콤보')?.price).toBe(22000);
  });

  it('matches ignoring spacing and case', () => {
    expect(lookupEstimatedPrice(' 스타 벅스 ', '카 페 라 떼')?.price).toBe(5000);
  });
