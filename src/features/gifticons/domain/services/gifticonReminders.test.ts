...
describe('syncGifticonReminders', () => {
  ...
  it('schedules reminders and writes their ids back on create', async () => {
    ...
    expect(mockedSchedule).toHaveBeenCalledWith(gifticon, [7, 3], 9);
    expect(mockedSetIds).toHaveBeenCalledWith('g1', ['n1', 'n2']);
  });

  it('passes the saved notification hour through to the scheduler', async () => {
    mockedGetHour.mockResolvedValue(21);
    await syncGifticonReminders({ gifticon, isOwner: true, isEditing: false });

    expect(mockedSchedule).toHaveBeenCalledWith(gifticon, [7, 3], 21);
  });
