...
- **위치 권한 사전 고지(prominent disclosure)** — OS 권한창 이전에 용도/기기 내 사용/
  백그라운드 미수집을 인앱 다이얼로그로 안내 (`src/shared/utils/location.ts`)
...
- 위치 권한 첫 요청 시 사전 고지 다이얼로그 → "계속" 눌러야 OS 권한창
...
- **데이터 안전** 양식 — `docs/privacy-policy.md` 2·3·7항과 일치하게: 위치, 사진, 이메일,
  앱 활동 수집 신고. 위치는 "선택·기능 목적" 으로. **데이터 삭제** 섹션에 인앱 삭제
  경로(`설정 → 계정 삭제`) + 문의 이메일 기재.
- **위치 권한 선언 양식** — 근처 매장 알림 용도, 정밀 위치(FINE) 사용, 백그라운드
  위치 없음, 인앱 사전 고지 스크린샷 첨부
...
- FINE 위치를 COARSE로 낮추는 건 하지 않음 — 300m 반경 근처 알림은 대략적 위치
