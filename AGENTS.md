...
- **Folder structure**: `src/features/<domain>/` holds everything for one domain (e.g. `gifticons`, `auth`), split into `components/`, `hooks/`, `screens/`, `services/`, and (where needed) `context/`. Cross-feature code goes in `src/shared/` — `{components,hooks,theme,utils,types}`, plus a module folder for a cross-cutting concern (e.g. `src/shared/auth/` holds the `AuthProvider` and the `useCurrentUser`/`useAuthActions`/`useAuthBootstrap` hooks that every feature and the root navigator consume). `src/app` holds root navigation/setup screens; `src/lib` holds external-service configuration (e.g. Firebase).
- **Large features split into sub-domain folders**: `gifticons/` is organised as `capture/` (add flow), `browse/` (home list), `detail/`, `reporting/` (spending report + expiry calendar), `geofence/` (nearby alerts), `gallery/` (photo auto-import), `ocr/`, `dev/` (dummy-data tooling), and `domain/` — the shared core (`types.ts`, `errors.ts`, value/filter/usage logic, plus `domain/{components,hooks,services}` for the data-access layer and widgets like `GifticonCard` used across slices). Each sub-domain folder holds its own screen + components + hooks + services, flat. A sub-domain may depend on `domain/` and on a clearly one-directional peer (e.g. `browse/` on `geofence/`), never the reverse. This is organisation only — the import-boundary rule treats the whole feature as one unit.
...
- **No import cycles** (`import/no-cycle`, `error`): the root stack's `RootStackParamList` lives in `src/app/navigationTypes.ts` (not `RootNavigator.tsx`) so screens and `navigationRef` can type against it without cycling back through the navigator. Put a shared type in its own module rather than importing it from a file that imports yours.
